...
@itemlist[
 ...
 @item{@DFlag{fix} applies safe fixes after applicability and idempotence checks.}
 @item{@DFlag{fix-preview} prints safe replacement previews to stderr without writing files.}
...
@itemlist[
 @item{trailing whitespace removal with source-line applicability checks}
 @item{missing final newline insertion with EOF applicability checks}
 @item{final @tt{#t} to @tt{else} replacement only when the syntax span and diagnostic agree}
]

@DFlag{fix} applies only fixes that pass applicability and idempotence checks.
@DFlag{fix-preview} prints each replacement to stderr without writing. Require,
provide, extract-let, and other heuristic transformations remain diagnostics
only until they have syntax spans and a safe replacement contract.
...
@itemlist[
 ...
 @item{Auto-fixes are limited to applicability-checked, idempotent replacements; use @DFlag{fix-preview} to inspect them without writing.}
]
